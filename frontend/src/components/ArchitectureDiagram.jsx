...
export default function ArchitectureDiagram() {
  ...
  const toggleBreaker = async (service, currentVal, setter) => {
    ...
    try {
      ...
      if (res.status === 403) {
        alert(
          data.detail || "Enable INAYAT_DEMO_MODE to toggle circuit breakers.",
        );
        return;
      }
    ...
    <section className="py-20 px-6 max-w-6xl mx-auto border-t border-cyber-border/40 relative">
      <div className="text-center mb-16">
        ...
        <p className="text-sm font-subheading text-cyber-muted max-w-xl mx-auto font-light">
          Click on any node to view sequence data flow. Adjust physical circuit
          breakers to test the self-healing fail-safe system.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-center">
        ...
        <div className="lg:col-span-2 relative flex justify-center bg-cyber-bg/50 border border-cyber-border/60 rounded-3xl p-6 overflow-hidden h-[340px] shadow-inner">
          <svg viewBox="0 0 800 300" className="w-full h-full object-contain">
            ...
            {/* Path linkages */}
            <path
              d="M 100 120 L 250 70 L 400 120 L 550 70 L 700 120"
              fill="none"
              stroke="url(#cyan-mag)"
              strokeWidth="2.5"
              strokeDasharray="6 4"
            ...
            {steps.map((step) => {
              ...
              const isBreakerActive =
                (step.id === 2 && breakerMem0) ||
                (step.id === 3 && breakerNeo4j);
      ...
      <div className="mt-12 p-8 rounded-2xl glass-glow-magenta grid grid-cols-1 md:grid-cols-2 gap-8 items-center border border-cyber-border shadow-2xl relative">
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-cyber-muted mt-1 leading-relaxed font-light">
              Manually blow system breakers to audit real-time health
              degradation and auto-recovery loops.
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-8 justify-around">
          ...
          <div className="flex items-center gap-4">
            <span
              className={`text-xs font-heading font-bold uppercase tracking-wider ${breakerMem0 ? "text-cyber-magenta" : "text-cyber-muted"}`}
            >
              Mem0 Switch
            </span>
            <button
            ...
            >
              <div
                className={`w-6 h-6 rounded-full bg-white transition-transform duration-300 flex items-center justify-center ${breakerMem0 ? "transform translate-x-6" : ""}`}
              >
                {breakerMem0 && (
                  <AlertTriangle className="w-3.5 h-3.5 text-cyber-magenta animate-pulse" />
                )}
              </div>
          ...
          <div className="flex items-center gap-4">
            <span
              className={`text-xs font-heading font-bold uppercase tracking-wider ${breakerNeo4j ? "text-cyber-magenta" : "text-cyber-muted"}`}
            >
              Neo4j Switch
            </span>
            <button
              onClick={() =>
                toggleBreaker("neo4j", breakerNeo4j, setBreakerNeo4j)
              }
              disabled={loading}
            ...
            >
              <div
                className={`w-6 h-6 rounded-full bg-white transition-transform duration-300 flex items-center justify-center ${breakerNeo4j ? "transform translate-x-6" : ""}`}
              >
                {breakerNeo4j && (
                  <AlertTriangle className="w-3.5 h-3.5 text-cyber-magenta animate-pulse" />
                )}
